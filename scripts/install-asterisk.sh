...
DEFAULT_API_BASE="https://dev1.publichost.cloud"
NODE_UUID=""
API_BASE=""
API_TOKEN=""
...
install_packages_debian() {
  ...
  if apt-cache show asterisk-codec-bcg729 >/dev/null 2>&1; then
    run "apt-get install -y --no-install-recommends asterisk-codec-bcg729"
  else
...
dsn=mnscloud
readsql=SELECT CONCAT('PJSIP/', CONCAT(COALESCE(r.VroPrepend, ''), SUBSTRING(dp.NormalizedNumber, IFNULL(r.VroStripDigits, 0) + 1)), '@', trunk_endpoint.id) FROM (SELECT caller_ext.VoipPabxAccountVpaUUID AS PabxUUID, caller_ext.UserUsrUUID AS UserUUID, IF(rule.VdrResultType = 'blocked', NULL, IF(rule.VdrReplacement IS NOT NULL AND TRIM(rule.VdrReplacement) <> '', IF(rule.VdrOperator = 'regex', REGEXP_REPLACE('\${SQL_ESC(\${ARG2})}', rule.VdrPattern, rule.VdrReplacement), rule.VdrReplacement), CONCAT(COALESCE(rule.VdrPrepend, ''), SUBSTRING('\${SQL_ESC(\${ARG2})}', IFNULL(rule.VdrStripDigits, 0) + 1)))) AS NormalizedNumber FROM AsteriskEndpoint caller JOIN VoipPabxExtension caller_ext ON caller_ext.VpeUUID = caller.VoipPabxExtensionVpeUUID JOIN VoipPabxAccount account ON account.VpaUUID = caller_ext.VoipPabxAccountVpaUUID JOIN VoipPabxDialPlan plan ON plan.VdpUUID = COALESCE(caller_ext.VoipPabxDialPlanVdpUUID, account.VoipPabxDialPlanVdpUUID) JOIN VoipPabxDialPlanRule rule ON rule.VoipPabxDialPlanVdpUUID = plan.VdpUUID AND rule.UserUsrUUID <=> caller_ext.UserUsrUUID WHERE '\${SQL_ESC(\${ARG1})}' LIKE CONCAT('PJSIP/', caller.id, '-%') AND caller_ext.VpeEnabled = 1 AND caller_ext.VpeDateDeleted IS NULL AND account.VpaIsActive = 1 AND account.VpaDateDeleted IS NULL AND plan.VdpEnabled = 1 AND plan.VdpDateDeleted IS NULL AND rule.VdrEnabled = 1 AND rule.VdrDateDeleted IS NULL AND rule.VdrDirection = 'outbound' AND rule.VdrResultType IN ('outbound', 'blocked') AND ((rule.VdrOperator = 'regex' AND '\${SQL_ESC(\${ARG2})}' REGEXP rule.VdrPattern) OR (rule.VdrOperator = 'exact' AND IF(rule.VdrCaseSensitive <> 0, '\${SQL_ESC(\${ARG2})}' = rule.VdrPattern, LOWER('\${SQL_ESC(\${ARG2})}') = LOWER(rule.VdrPattern))) OR (rule.VdrOperator = 'prefix' AND IF(rule.VdrCaseSensitive <> 0, '\${SQL_ESC(\${ARG2})}' LIKE CONCAT(rule.VdrPattern, '%'), LOWER('\${SQL_ESC(\${ARG2})}') LIKE CONCAT(LOWER(rule.VdrPattern), '%')))) ORDER BY rule.VdrPriority ASC, rule.VdrDateCreated ASC LIMIT 1) dp JOIN VoipPabxOutboundRoute r ON r.VoipPabxAccountVpaUUID = dp.PabxUUID AND r.UserUsrUUID <=> dp.UserUUID JOIN VoipPabxTrunk trunk ON trunk.VptUUID = r.VoipPabxTrunkVptUUID AND trunk.UserUsrUUID <=> r.UserUsrUUID JOIN AsteriskEndpoint trunk_endpoint ON trunk_endpoint.VoipPabxTrunkVptUUID = trunk.VptUUID WHERE dp.NormalizedNumber IS NOT NULL AND dp.NormalizedNumber REGEXP r.VroPattern AND r.VroEnabled = 1 AND r.VroDateDeleted IS NULL AND trunk.VptEnabled = 1 AND trunk.VptDateDeleted IS NULL AND trunk.VptDirection IN ('outbound', 'both') ORDER BY r.VroPriority ASC, r.VroDateCreated ASC LIMIT 1
