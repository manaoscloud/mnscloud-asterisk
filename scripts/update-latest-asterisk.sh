...
usage() {
  cat <<'USAGE'
Usage:
  sudo ./scripts/update-latest-asterisk.sh [--api-base <api-v1-url>] [--channel stable] [--print-command]

Resolves the latest approved Asterisk release from the MNSCloud control plane.
If this product is not yet in the registry, it falls back to the latest
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h) usage; exit 0 ;;
    *) printf '[update-latest-asterisk] ERROR: unknown argument: %s\n' "$1" >&2; usage >&2; exit 2 ;;
  esac
done
...
resolve_ref() {
  ...
  ref="$(
    MNSCLOUD_RELEASE_URL="${normalized_api_base}/runtime/releases/latest?product=mnscloud-asterisk&channel=${CHANNEL}" python3 <<'PY' 2>/dev/null || true
import json
import os
...
[[ "$REF" =~ ^v[0-9]+[.][0-9]+[.][0-9]+([-+][0-9A-Za-z.-]+)?$ ]] || {
  printf '[update-latest-asterisk] ERROR: invalid latest %s ref: %s\n' "$CHANNEL" "${REF:-empty}" >&2
  exit 1
}

printf '[update-latest-asterisk] latest release: %s\n' "$REF"
if [[ "$PRINT_COMMAND" == '1' ]]; then
  printf 'cd %s\nsudo ./scripts/update-latest-asterisk.sh\n' "$ROOT_DIR"
  exit 0
fi

bash "${SCRIPT_DIR}/update-asterisk.sh" --ref "$REF"
