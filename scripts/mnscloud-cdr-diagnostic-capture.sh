...
usage() {
  cat >&2 <<'USAGE'
Usage: mnscloud-cdr-diagnostic-capture.sh --enabled yes --module pabx --engine asterisk --resource-type pabx_cdr --resource-uuid <id> --call-id <call-id> [--mode sip_capture|pcapng] [--interface any] [--duration 60] [--filter "port 5060"]
